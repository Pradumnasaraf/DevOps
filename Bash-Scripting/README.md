...
It is a placeholder for a value. Just like any other programming language. We use $ to access the variable. Eg: `$NAME`. We can also use `${NAME}`. The braces are used to make sure that the variable is not mistaken for a command.
...
But here is one more catch. If we exit the terminal and open a new one, the variable will be gone. We can add it to the `.bashrc` file to make it permanent. It is a hidden file in the home directory. We can open it by `vi ~/.bashrc` or any other editor. We can add the variable to the file. Eg: `export NAME="John"`.
...
Will print out the specified key.
